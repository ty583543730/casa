...
class Log
{
     ...
     */
    public static function logger($mark, $log_content, $keyp = "")
    {
        $max_size = 20000000;
        $path = RUNTIME_PATH . 'log/' . date('Ymd');
        if (!file_exists($path)) {
            mkdir($path, 0777, true); //修改权限
        }
        if ($keyp == "") {
            $log_filename = RUNTIME_PATH . 'log/' . date('Ym-d') . ".txt";
        } else {
            $log_filename = $path . '/' . $keyp . ".txt";
        }
        if (file_exists($log_filename) && (abs(filesize($log_filename)) > $max_size)) {
            rename($log_filename, dirname($log_filename) . DS . date('Ym-d-His') . $keyp . ".txt");
        }
        $t = microtime(true);
