...
class Image {
     ...
     * @param $key
     */
    static function genCode($key, $width = 48, $height = 25) {
        ob_end_clean();
        $randval = SString::randString(4, 1);
        if (isset($_GET["deviceId"])) {
            $client = $_GET["deviceId"];
            cache($key . $client, $randval, 300);
        } else if (isset($_SERVER["HTTP_DEVICEID"])) {
            $client = $_SERVER["HTTP_DEVICEID"];
            cache($key . $client, $randval, 300);
        } else {
            session($key, $randval);
        }
        //app访问，通过header传过来的deviceId来区分。
        Image::genImage($randval, 4, 1, 'png', $width, $height);
        return $randval;
    }

    /**
     * @param $key校验验证码
     * @param $refresh 是否要清空当前检验码，默认是检验一次就清除
     */
    static function checkCode($key, $code, $refresh = true) {
        if (isset($_GET["deviceId"])) {
            $client = $_GET["deviceId"];
        } else if (isset($_SERVER["HTTP_DEVICEID"])) {
            $client = $_SERVER["HTTP_DEVICEID"];
        ...
        $randval = session($key);
        if (!$randval && isset($client)) {
            // app 访问，session是不固定的，通过header传过来的deviceId来区分。
                $randval = cache($key . $client);
        }else{
           return false;
        }
        if ($refresh) {
            session($key, null);
            if ($client) {
                cache($key . $client, null);
            }
        }
        return $randval && $code && $randval == $code;
    }
    ...
    /**
     * @param string $verifyName 生成校验图
     */
    static function genImage($randval, $length = 4, $mode = 1, $type = 'png', $width = 48, $height = 22) {

        Image::buildImageVerify($length, $mode, $type, $width, $height, $randval);
    }
     ...
     * @return string
     */
    static function buildImageVerify($length = 4, $mode = 1, $type = 'png', $width = 48, $height = 22, $randval) {
    ...
    static function GBVerify($length = 4, $type = 'png', $width = 180, $height = 50, $fontface = 'simhei.ttf', $verifyName = 'verify') {
        ...
        $code = SString::randString($length, 4);
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            $codex = SString::msubstr($code, $i, 1);
