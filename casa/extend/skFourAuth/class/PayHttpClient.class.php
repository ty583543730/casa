...
class PayHttpClient
{
     ...
     *
     */
    function call($resource = "",$data = "",$submitUrl = "")
    {
        //启动一个CURL会话
        $ch = curl_init();
        // 设置curl允许执行的最长秒数
...
//        dd($this->responseCode);
        if ($res == NULL) {
            ...
            curl_close($ch);
            Log::logger($resource. "错误1", JSONReturn($this->errInfo), "skFourAuth");
            return false;
        } else if ($this->responseCode != "200") {
            $this->errInfo = "call http err httpcode=" . $this->responseCode;
            curl_close($ch);
            Log::logger($resource. "错误2", JSONReturn($this->errInfo), "skFourAuth");
            return false;
        }
        curl_close($ch);
        $this->resContent = $res;
        Log::logger($resource. "请求响应数据", $res, "skFourAuth");
        return true;
    }
